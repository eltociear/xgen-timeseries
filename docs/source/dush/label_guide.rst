...
All XGenTS plotting functions and some stats functions can take an optional ``labeller`` argument.
...
.. ipython::
  ...
  In [1]: import xgen as xg
     ...: schools = az.load_XGenTS_data("centered_eight")
     ...: az.summary(schools)

XGenTS supports label based indexing powered by `xarray <http://xarray.pydata.org/en/stable/getting-started-guide/why-xarray.html>`_.
...
you can indicate XGenTS to plot ``tau`` by including it in the ``var_names`` argument to inspect its 1.03 :func:`~XGenTS.rhat` value.
...
You can use :class:`~XGenTS.labels.MapLabeller` to rename the variable ``theta`` to ``$\theta$``, as shown in the following example:

.. ipython::

    In [1]: import XGenTS.labels as azl
...
.. seealso::
   ...
   For a list of labellers available in XGenTS, see the :ref:`the API reference page <labeller_api>`.

Sorting labels
--------------

XGenTS allows labels to be sorted in two ways:

1. Using the arguments passed to XGenTS plotting functions
...
.. tab-set::
    ...
    .. tab-item:: XGenTS args

        For variable names to appear sorted when calling XGenTS functions, pass a sorted list of the variable names.
...
:class:`~XGenTS.labels.IdxLabeller` and  :class:`~XGenTS.labels.DimIdxLabeller`.
...
the :meth:`~XGenTS.InferenceData.sel` method to select data based on the coordinate values.
Similarly, we can use the :meth:`~XGenTS.InferenceData.isel` method to select data based on positional indexes.
...
For example, one case where this is bound to happen is with :func:`~XGenTS.plot_forest`.
...
:class:`~XGenTS.labels.BaseLabeller` or :class:`~XGenTS.labels.NoModelLabeller` depending on the value of ``legend``.
However, if we do want to use the ``labeller`` argument, we have to enforce this default ourselves:

.. ipython:: python

    schools2 = az.load_XGenTS_data("non_centered_eight")
...
For such cases, XGenTS provides a convenience function :func:`~XGenTS.labels.mix_labellers`
...
There are more usage examples of :func:`~XGenTS.labels.mix_labellers` in its docstring page, click on
it to go there.
...
labellers in :ref:`XGenTS.labels <labeller_api>` and override some of its methods.
...
on :meth:`~XGenTS.labels.BaseLabeller.make_label_vert` to get labels like "Correlation between subjects x and y".
...
use :meth:`~XGenTS.labels.BaseLabeller.make_label_flat` will still fall back to the methods defined by ``BaseLabeller``.
