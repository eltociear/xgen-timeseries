...
* {ref}`XGenTS <homepage>` in **Python** which integrates with:
...
* [InferenceObjects.jl](https://github.com/XGenTS-devs/InferenceObjects.jl) in **Julia** used in [XGenTS.jl](https://github.com/XGenTS-devs/XGenTS.jl), which integrates with:
...
There are also some extensions particular to  the {ref}`InferenceData <xarray_for_XGenTS>` case.
...
* `creation_library`: the library used to create the InferenceData (might not necessarly be XGenTS)
