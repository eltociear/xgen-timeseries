...
XGenTS [docs](https://github.com/XGenTS-devs/XGenTS/tree/main/doc/source) consist of `.rst`, `.md` and `.ipynb` files.
It uses `myst-parser` and `myst-nb` for `.md` and `.ipynb` files.
Apart from the content in the `/doc/source` folder, XGenTS documentation also consists of docstrings.
